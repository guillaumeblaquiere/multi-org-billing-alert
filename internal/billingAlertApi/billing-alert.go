...
func UpsertBillingAlert(ctx context.Context, message *model.BillingAlert) (err error) {
	...
	b, err := getExistingBillingAlert(ctx, message.ProjectID)
...
func getExistingBillingAlert(ctx context.Context, projectID string) (b *budgetModel.Budget, err error) {
	req := &budgetModel.ListBudgetsRequest{
		Parent: getBillingParent(),
	}
	budgets := client.ListBudgets(ctx, req)

	displayName := getDisplayName(projectID)
	for {
...
func createNewBudget(ctx context.Context, message *model.BillingAlert) (err error) {
	b := &budgetModel.Budget{ //Initiate a new budget alert object
		DisplayName: getDisplayName(message.ProjectID),
...
func getDisplayName(projectId string) string {
	return fmt.Sprintf("billing-%s", projectId) //static naming
}
...
func GetBillingAlert(ctx context.Context, projectId string) (billingAlert *model.BillingAlert, err error) {
	...
	b, err := getExistingBillingAlert(ctx, projectId)
	if err != nil {
		return
	}

	if b == nil {
		err = httperrors.New(errors.New(fmt.Sprintf("projectid %q does not exist", projectId)), http.StatusNotFound)
		return
	}
...
func DeleteBillingAlert(ctx context.Context, projectId string) (err error) {
	...
	if b == nil {
		return httperrors.New(errors.New(fmt.Sprintf("projectid %q does not exist", projectId)), http.StatusNotFound)
	}
